...
PowerShell 7（`pwsh`）では `[ESC]` を `` `e `` と表現できます。`Write-Host` コマンドレットで出力してみましょう。[^0]

![write-host-sixel7](./img/write-host-sixel7.png)

[^0]: 従来の PowerShell でも `[char]27` と表現できます。![write-host-sixel](./img/write-host-sixel.png)
